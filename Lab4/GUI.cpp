...
static TCHAR szTitle[] = _T("Store Management");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    PAINTSTRUCT ps;
    HDC hdc;
    TCHAR greeting[] = _T("Welcome to store manager!");
        ...
        case 1:
            {
                ...
                {
                    int result = MessageBox(hWnd, value, "Are you sure you want to provide this path to Sold?", MB_YESNO |
                        MB_TOPMOST | MB_ICONWARNING);
                    switch (result)
                    {
                    case IDYES:
                        {   string buffer = value;
                            MessageBox(hWnd, "Path Provided to Stoc", "YES", MB_ICONINFORMATION);
                            createFile("C:\\Users\\Asihma\\CSSO\\Lab4\\output_sold.txt", GENERIC_ALL);
                            WriteToFile("C:\\Users\\Asihma\\CSSO\\Lab4\\output_sold.txt", buffer.c_str());
        ...
        case 2:
            {
                ...
                {
                    int result = MessageBox(hWnd, value, "Are you sure you want to provide this path to Deposit?", MB_YESNO |
                        MB_TOPMOST | MB_ICONWARNING);
                    switch (result)
                    {
                    case IDYES:
                        {
                            MessageBox(hWnd, "Path Provided to Deposit", "YES", MB_ICONINFORMATION);
                            string buffer = value;
                
                            createFile("C:\\Users\\Asihma\\CSSO\\Lab4\\output_deposit.txt", GENERIC_ALL);
                            WriteToFile("C:\\Users\\Asihma\\CSSO\\Lab4\\output_deposit.txt", buffer.c_str());
                        }
                        break;
                    case IDNO:
                        MessageBox(hWnd, "You pressed NO", "NO", MB_ICONINFORMATION);
        ...
        case 3:
            {
                if (!isProcessRunning)
                {
                    char testare[] = "C:\\Users\\Asihma\\CSSO\\Lab4\\management.exe";
                    STARTUPINFO si;
